...
class Main:
    def __init__(self, task_list, options, modified_sites=[], immutable_tasks=[], flg_exclude_strand="off", max_sites_for_verification=3000, 
        ...
        self.BLAST_bin = os.path.join(BASE_DIR, "lib", "bin")
    ...
    # gbk_files = either a list of gbk_files, or [seqlist1, seqlist2,...]
    def process(self, gbk_files=[], immutable_tasks=[], modified_sites=[], top_indend=50, flg_exclude_strand=None, verified=False):
        ...
        for i in range(len(filelist)):
            ...
            genome_item = filelist[i]
            if isinstance(genome_item, str):    # filename
                fname = genome_item
                genome = self.oIO.get_SeqDataset(fname)
            else:
                genome = genome_item
                fname = genome.get_input_file_name()
                
            # Process list of sequences
            seqlist_output = self.process_seqlist(seqlist={genome.get_description() : genome}, 
                step=self.bigstep, top_indend=top_indend, modified_sites=modified_sites, verified=verified)
    ...
    def report_output(self, seq, seqname, acc, top_indend=150, tasks=None, genes=None, verified=False):
        ...
        for task in tasks:
            ...
            if task == "MOD" and self.flg_distribution_stat in tasks[task]:
    ...
    def get_stat_report(self):
        return "" if self.oStatReport is None else self.oStatReport.get() 
    ...
    def getSeqFromFASTA(self,fname,names=[]):
        oFASTA, contigs = self.IO.parse(fname,"fasta")
        return {oFASTA.description:{"sequence":oFASTA.seq,
            "dataset" : {'contigs' : contigs,
            "Accession" : oFASTA.description,
            "Gene map" : []}
            }}
    ...
    def identify_GenomicIslands(self, seq, window=None, bigstep=None, smallstep=None, ref_pattern=None, tasks=None,
        ...
        def parse_GI_record(ls):
            # Expectation: [start, stop, stat] or ['start-stop', start]
            if len(ls) == 3:
                return [f"{ls[0]}-{ls[1]}",ls[2]]
            elif len(ls) == 2:
                # check that the first value is 'start-stop'
                try:
                    start, stop = [int(v) for v in ls[0].split("-")]
                    return [f"{start}-{stop}",ls[1]]
                except:
                    pass
            sys.exit(f"Line 794 in GI_finder: Unproperly formatted GI record {ls}")
        ...
        tasks[task]['frames'] = [parse_GI_record(ls) for ls in loci]
...
class Pattern:
    ...
    def setMatrix(self, strSeq):
        ...
        for l in letters:
            self.boxAGCT[l] = strSeq.count(l)/len(strSeq)
        return 1
...
class Interface:
    ...
    def execute(self, GBK_files=[], top_indend=150, modified_sites=[], motif_setting="", verified=False):
        if GBK_files:
            GBK_files = GBK_files if isinstance(GBK_files,list) else [GBK_files]
        else:
            GBK_files = [self.options['-g']]
        # Check if the list of input sequence files corresponds to the list of modified files
        if len(GBK_files) != len(modified_sites):
        ...
        self.output = oMainWin.process(GBK_files, immutable_tasks=self.immutable_tasks, top_indend=top_indend, 
    ...
    def validate(self, options, scenario):
        ...
        if self.options['-g']:
            for fname in self.options['-g']:
                if not isinstance(fname, dict) and not os.path.exists(fname):
        ...
        for path in self.options['-g']:
            if not isinstance(path, dict) and not os.path.exists(path):
