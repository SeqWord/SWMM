...
class BLAST(container):
    ...
    def clean(self, tmp_file_identifier = "~"):
        # Remove tmp files from source directory
        for fname in [fn for fn in os.listdir(self.source_path) if fn.startswith(tmp_file_identifier)]:
            os.remove(os.path.join(self.source_path, fname))
            
        parent_dir = os.path.dirname(self.source_path)
        # Remove tmp files from parent directory
        for fname in [fn for fn in os.listdir(parent_dir) if fn.startswith(tmp_file_identifier)]:
